...
def prepare_macro_data(base_paths):
    ...
    Notes
    -----
        - 所有輸入的 CSV 檔案需有一個名為 'date' 的 column，用於進行數據合併。
    ...
    column_names = ['date', 'lag1_cpi', 'lag1_cpi_grate',
                    'lag1_ipi', 'lag1_ipi_grate', 'lag1_un_rate',
                    'lag1_M1B', 'lag1_M1B_grate', 'lag1_new_issue']
...
def get_daily_anue_senti():
    anue_data = pd.read_csv(data_dir / 'anue_news_data' / 'anue_clear(256721).csv')
